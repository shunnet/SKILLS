...
> - **地址自动组包**：`Packer` / `PackerAsync` / `UnPacker` / `UnPackerAsync`（IPacker 契约）——继承 DaqAbstract 即内置，公开字段 `packerHandler` / `bytesHandler` 可直接使用；四个 `UnPacker`/`UnPackerAsync` 重载均带 `bool isStringReverseByteWord = false`（v26.235.2+：`String` 解包按 16 位字反转字节，连接级配置如欧姆龙 FINS）
